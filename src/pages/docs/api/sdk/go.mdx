...
The `vulnerability` package handles vulnerability data. Note that `GetVulnerability` returns an `nvd.CveItem` (the full NVD record), not a `sirius.Vulnerability`:

```go
// Get vulnerability information (returns nvd.CveItem)
cveItem, err := vulnerability.GetVulnerability("CVE-2021-1234")
...
// Retrieve a value (returns ValkeyResponse)
resp, err := kvStore.GetValue(ctx, "scan-status")
val := resp.Message.Value
```
...
Connection strings for infrastructure services are defined as package-level constants:

- `SIRIUS_RABBITMQ` (`sirius/queue`): RabbitMQ connection string (default: `amqp://guest:guest@sirius-rabbitmq:5672/`)
- `SIRIUS_VALKEY` (`sirius/store`): Valkey store address (default: `sirius-valkey:6379`)

The database connection is configured automatically based on the runtime environment (Docker vs local).
