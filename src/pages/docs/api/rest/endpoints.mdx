...
1. Use the Go SDK's `store` package to query Valkey directly
2. Monitor the RabbitMQ message queue for scan updates
...
>
  > **Note:** This endpoint is planned for future implementation. Scan results are currently stored in the PostgreSQL database and are accessible through the host and vulnerability endpoints.
...
Sirius Scan integrates with the [National Vulnerability Database](https://nvd.nist.gov/) to enrich vulnerability data. The Go SDK's `nvd` package handles NVD lookups internally -- you do not need to query the NVD API directly.
